...
El bootstrap también instala `eza`, `fzf`, `bat` y `fresh-editor`, más
aliases de Fish en `~/.config/fish/conf.d/` sin sobrescribir tu configuración:
`ls`, `ll`, `la`, `lt` con eza y `cat` con bat. `fzf` queda instalado para
usarlo después.
